...
export const Component = () => {
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style relative bg-black text-white overflow-hidden" style={{ minHeight: '300vh' }}>
      <canvas ref={canvasRef} className="hero-canvas fixed top-0 left-0 w-full h-full" style={{ zIndex: 0 }} />
      ...
      <div ref={menuRef} className="side-menu fixed left-8 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-8" style={{ visibility: 'hidden' }}>
        ...
        <div className="vertical-text text-sm tracking-[0.5em] origin-center -rotate-90 text-white/50">SPACE</div>
      </div>

      {/* Main content */}
      <div className="hero-content cosmos-content fixed inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
        {currentSection === 0 && (
          <>
            <h1 ref={titleRef} className="hero-title text-7xl md:text-[10rem] font-bold tracking-[0.2em] mb-4 text-center">
              {splitTitle('HORIZON')}
            </h1>
            
            <div ref={subtitleRef} className="hero-subtitle cosmos-subtitle text-center text-lg md:text-2xl text-white/70 tracking-widest uppercase font-light">
      ...
      <div ref={scrollProgressRef} className="scroll-progress fixed right-8 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-4" style={{ visibility: 'hidden' }}>
        <div className="scroll-text text-xs tracking-widest text-white/50 [writing-mode:vertical-rl]">SCROLL</div>
        <div className="progress-track w-[2px] h-32 bg-white/10 relative">
          <div 
            className="progress-fill absolute top-0 left-0 w-full bg-white transition-all duration-300" 
            style={{ height: `${scrollProgress * 100}%` }}
          />
        </div>
        <div className="section-counter text-xs text-white/50 tracking-widest">
          {String(currentSection + 1).padStart(2, '0')} / {String(totalSections + 1).padStart(2, '0')}
        </div>
      </div>

      {/* Additional sections for scrolling */}
      <div className="scroll-sections absolute top-0 left-0 w-full z-10" style={{ height: '300vh' }}>
       {[...Array(2)].map((_, i) => {
          const titles = {
            0: 'HORIZON',
            1: 'COSMOS',
          ...
          return (
            <section key={i} className={`content-section h-screen w-full flex flex-col items-center justify-center transition-opacity duration-1000 ${currentSection === i + 1 ? 'opacity-100' : 'opacity-0'}`} style={{ position: 'absolute', top: `${(i + 1) * 100}vh` }}>
              <h1 className="hero-title text-7xl md:text-[10rem] font-bold tracking-[0.2em] mb-4 text-center">
